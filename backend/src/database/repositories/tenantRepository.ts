...
class TenantRepository {
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Address(options.database).deleteMany({ tenant: id }, options);

    await Person(options.database).deleteMany({ tenant: id }, options);

    await WaterReading(options.database).deleteMany({ tenant: id }, options);

    await Device(options.database).deleteMany({ tenant: id }, options);
